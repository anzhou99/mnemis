...
READ_FILE_SCHEMA = ToolSchema(
    name="read_file",
    description="读取本地工作区内的文件内容。只能读取 workspace/ 目录下的文件。",
    input_schema={
        "type": "object",
        "properties": {
            "filename": {
...
WRITE_FILE_SCHEMA = ToolSchema(
    ...
    input_schema={
        ...
        "properties": {
            "filename": {
                "type": "string",
                "description": "必填。文件名（不含路径），如 'notes.md'",
            },
...
def execute_write_file(filename: str, content: str) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            tool_use_id="", content=f"✓ 已写入 {filename}（{len(content)} 字符）"
        )
    except Exception as e:
